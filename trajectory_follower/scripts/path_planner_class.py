...
class Path_Planner():
    ...
    def move_to(self, x_goal, y_goal):
        plt.plot(x_goal, y_goal, 'go', ms=4)
        self.x_goal = x_goal
        self.y_goal = y_goal
        x0, y0 = self.drive.posn[0], self.drive.posn[1]
    ...
    def follow(self):
        ...
        y_range = np.array([self.f(x_) for x_ in x_range])
        plt.plot(x_range, y_range, 'r-')
        zeros = np.zeros(int((self.x_max-self.x_min)/delta))
        if zeros.size > x_range.size:
            while(zeros.size > x_range.size):
                zeros = zeros[:-1]
        elif zeros.size < x_range.size:
            while(zeros.size < x_range.size):
                x_range = x_range[:-1]
        plt.plot(zeros, x_range, 'k-')
        plt.plot(x_range, zeros, 'k-')
        if show_here: plt.show()
